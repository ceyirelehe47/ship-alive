...
14. **短基准必须进程内交错 A/B**：本机 CPU 频率爬升会让跨运行对比严重
    失真（同一份代码测出 10k–56k paths/s）。把旧实现复制进基准测试交替跑
    （见 `tests/perf_jobs.rs` 的 legacy 模块），才是可信的加速比。
15. **帧级共享索引要惰性构建**：扫描索引若每步无条件构建，多数"没人扫描"
    的步骤纯付构建成本（曾让 churn 基准倒退 40%）。先 `any(本步有人要扫描)`
    预检再构建；领取时从共享表移除 = 旧的 local_claims 复查。
16. **英语 `label()` 系列是控制台/测试基线**：autotest 打印、既有断言
